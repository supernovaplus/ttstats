              ...
              {server.playersData.map((player, index) => (
                <tr key={index} className="odd:bg-kebab-odd even:bg-kebab-even hover:hover:bg-kebab-dk">
                  ...
                  <td data-label="Name #ID">
                    ...
                    {player[4] && <span className={'p-1 bg-red-800 ml-1 rounded text-xs text-white'}>Staff</span>}
                    {player[6] && <span className={'bg-orange-700 p-1 rounded ml-1 text-xs text-white'}>Donator</span>}
                  </td>
