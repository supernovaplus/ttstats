...
const Category = ({ children }: { children: ReactNode }) => {
  return <li className="font-sans bg-gray-500 dark:bg-kebab-btn mb-2 text-white select-none">{children}</li>;
};
...
const OneItem = ({ children, to = '/404' }: any) => (
  <li>
    ...
    <NavLink
      to={to}
      className="navlink block border-l-2 -ml-px-2 border-transparent hover:border-slate-600 dark:hover:border-slate-500 dark:hover:text-gray-400 select-none">
...
export default function Sidebar() {
  return (
    <>
      <aside className="w-fullbox-border bg-white dark:bg-kebab-bg-dm w-full min-w-[140px] sm:max-w-[200px] text-black dark:text-white h-fit dark:shdw text-center mb-3 rounded-tl-md overflow-hidden border-r border-r-kebab-bg">
