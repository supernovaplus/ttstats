...
export default function HighestID () {
	...
	const playersList = Object.values(servers).reduce((acc, server) => 
			server.loaded && server.playersData ? [...acc, ...server.playersData.filter(player => player[2] !== "?")] : acc, [])
		.sort((a,b) => b[2] - a[2])
		.map((data, index) => ({
			index: index+1, 
			name: data[0], 
	...
	const sortedList = playersList.length > 25 ? 
