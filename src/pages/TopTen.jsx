...
function TopBlock({ board, index, savedTop10Statuses }) {
	...
	return (
		<div className="border-start selected-border">
			<div className="border-title button-title text-center">{board.title}</div>
...
export default function Top10() {
	...
	const onSelectTop = ({ target }) => {
		setState(s => ({...s, selectedTop: parseInt(target.value) || 0}))
	};
		...
		<div className="container">
			...
			{state.data && (
				<>
					...
					<TopBlock key={state.selectedTop} board={state.data[state.selectedTop]} index={state.selectedTop}/>

					{/* {state.data.map((board, index) => (
						<TopBlock key={index} board={board} index={index} savedTop10Statuses={savedTop10Statuses} />
					))} */}
					<div className="border-start">
						<div className="border-title text-center">
							Leaderboards updates every hour
							<br />
