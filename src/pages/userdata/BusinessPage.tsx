...
export default function BusinessPage() {
  ...
  const filteredBusineses = businessData
    ...
    .map((biz, index) => {
      const owned = state.bizTiers?.data?.businesses.hasOwnProperty(biz.id);
  ...
  const onServerChange = (e: ChangeEvent<HTMLSelectElement>) => {
    setState((s) => ({ ...s, bizData: initialBizState.bizData, bizTiers: initialBizState.bizTiers, selectedServerEndpoint: e.target.value }));
  };

  const onGetTiersClicked = (e: MouseEvent<HTMLInputElement>) => {
    e.preventDefault();
    clearMessages();

    if (!selectedServerState) return;
    if (!state.selectedUserId) {
      addMessage('errors', 'UserId Missing');
      return;
    }

    setState((s) => ({ ...s, bizTiers: { loading: true, data: null } }));
    ...
    fetch(
      ...
      .then((res) => {
        if (res && res.businesses) {
          setState((s) => ({ ...s, bizTiers: { loading: false, data: res } }));
        } else if (res.error) {
      ...
      .catch((err: DOMException) => {
        addMessage('errors', 'Error: ' + err.message);
        setState((s) => ({ ...s, bizTiers: { loading: false, data: null } }));
      });
  };
    ...
    <ContentBlock title="Business">
      <div style={{ minHeight: '200px', maxHeight: '90vh' }}>
        <select
          ...
          className="text-black px-2 cursor-pointer w-full text-center"
        ...
        <div className="p-1 mt-2 text-white">
          <div className="flex gap-1 bg-slate-500 flex-wrap justify-center py-1 w-full mb-1">
            {state.bizTiers?.data?.businesses ? (
              <>
                ...
                <div className="border px-2">
                  my raw bonus: <MarkValue>{shortenLargeMoney(myBonus * 8)}</MarkValue> per day
                </div>
              </>
            ) : (
              <>
