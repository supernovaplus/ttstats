	...
	const handlePlayerFinderSubmit = (isButton = true) => {
		...
		for (const key in servers) {
			...
			servers[key].playersData.forEach((player) => {
				...
				if (playername.toLowerCase().includes(input) && (state.serverSelect === "All Servers" || state.serverSelect === servers[key].name) && (state.jobSelect === "All Jobs" || state.jobSelect === player[5])) {
					playerFinderFound.push([playername, servers[key]["endpoint"], servers[key]["sname"], player[3], player[5]]);
				}
			});
		}
		...
		setState((s) =>
			playerFinderFound.length > 0
				? { ...s, playerFinderMessages: "Found " + playerFinderFound.length + " player" + (playerFinderFound.length === 1 ? "" : "s"), playerFinderFound }
				: {
						...s,
					...
					<form id="playerFinderForm">
						...
						<div>
							...
							<select id="serverSelector" onChange={handleServerSelect} value={state.serverSelect} className="bold cursor">
								<option value="All Servers" className="bold">
									All Servers
								</option>
								{Object.values(servers).map((server, index) =>
									server.loaded ? (
										<option key={index} value={server.name} className="bold">
											{server.name}
										</option>
									) : (
										""
